# Xamarin.iOS - Swift Framework Binding Walkthrough
...
1. Open Xcode and create new Swift Framework which will be a proxy between Xamarin.iOS code and 3d party Swift framework. Click File -> New -> Project and follow the wizard steps:
    ...
    ![xcode create framework project](SolutionItems/xcode-create-framework-project.png)
...
1. Ensure that the **"Do Not Embed"** option is selected as we are going to control it manually later:
    ...
    ![xcode donotembed option](SolutionItems/xcode-donotembed-option.png)
...
1. Ensure that the Build Settings option **"Always Embed Swift Standard Libraries"** which includes Swift libraries with the framework is set to No. We are going to manually control which Swift dylibs are included into the final package:
    ...
    ![xcode always embed false option](SolutionItems/xcode-alwaysembedfalse-option.png)
...
1. Ensure that the **"Enable  Bitcode"** option is set to **No**. As of right now Xamarin.iOS doesn't include Bitcode while Apple requires all libraries to support the same architectures:
    ...
    ![xcode enable bitcode false option](SolutionItems/xcode-enablebitcodefalse-option.png)
...
1. Ensure that the "Objective-C Generated interface Header Name" option is enabled and specifies a header name. The default name is `<FrameworkName>-Swift.h`
    ...
    ![xcode objectice-c header enabled option](SolutionItems/xcode-objcheaderenabled-option.png)
...
1. Change scheme build configuration from **Debug** to **Release**. In order to do that, open the `Xcode -> Target -> Edit Scheme` dialog and then set the Build Configuration option to Release:
    ...
    ![xcode edit scheme](SolutionItems/xcode-edit-scheme.png)

    ![xcode edit scheme release](SolutionItems/xcode-edit-scheme-release.png)
...
1. Open Visual Studio for Mac and create a new Xamarin.iOS Binding Library project, give it a name, in our case SwiftFrameworkProxy.Binding and complete the wizard. The Xamarin.iOS binding template is located by the following path: iOS -> Library -> Binding Library:
    ...
    ![visualstudio create binding library](SolutionItems/visualstudio-create-binding-library.png)
...
1. Copy metadata generated by Sharpie at one of the previous steps, select the following Build Action in the properties window: ObjBindingApiDefinition for the "ApiDefinitions.cs" file and ObjBindingCoreSource for the "StructsAndEnums.cs" file:
    ...
    ![visualstudio project structure metadata](SolutionItems/visualstudio-project-structure-metadata.png)
...
1. Add native reference to the generated earlier fat framework, as well as each dependency of that framework. In our case we should add both SwiftFrameworkProxy and Gigya framework native references to the binding project.
    ...
    ![visualstudio project structure nativerefs](SolutionItems/visualstudio-project-structure-nativerefs.png)
    ...
    - Update properties of every native reference and check 3 important options
        ...
        ![visualstudio nativeref proxy options](SolutionItems/visualstudio-nativeref-proxy-options.png)
...
1. Create Xamarin.iOS project. You can use the iOS -> App -> Single View App as a starting point
    ...
    ![visualstudio app new](SolutionItems/visualstudio-app-new.png)
...
1. Add a binding project reference to the target project or .dll created previously. Treat the binding library as a regular Xamarin.iOS library
    ...
    ![visualstudio app refs](SolutionItems/visualstudio-app-refs.png)
...
2. In order to submit the app to the app store you want to use Xcode and distribute option which will update the .ipa and Swift SupportFolder .dylibs so it will be accepted by the AppStore:
    ...
    ![visualstudio archive for publishing](SolutionItems/visualstudio-archiveforpublishing.png)
    ...
    ![visualstudio archives](SolutionItems/visualstudio-archives.png)
...
3. This step is optional but it's really important to verify that your app can run on iOS 12.1 and earlier as well as 12.2. You can do it with help of Test Cloud and UITest framework. Create a UITest project and a simple UI test which runs the app.
    ...
    - Verify the result. In the AppCenter portal, navigate to the App -> Test -> Test runs
         ...
         ![visualstudio appcenter uitest result](SolutionItems/visualstudio-appcenter-uitest-result.png)

        From there select the desired test run and verify the result:

        ![visualstudio appcenter uitest runs](SolutionItems/visualstudio-appcenter-uitest-runs.png)
